...
func setUpTables() {
	...
	createTable("CREATE TABLE IF NOT EXISTS orders(id uuid, product_id uuid, name text, address text, house text, pin int, city text, phone VARCHAR(13), payment text, total int, status VARCHAR(10),FOREIGN KEY(product_id) REFERENCES product(id));","order")
