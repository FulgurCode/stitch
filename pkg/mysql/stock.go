...
func AddStock(stock models.Stock) error {
	var query = fmt.Sprintf("INSERT INTO stock(product_id, s,m,l,xl,xxl,xxxl) VALUES('%s',%d,%d,%d,%d,%d,%d,s + m + l + xl + xxl + xxxl);", stock.ProductId, stock.S, stock.M, stock.L, stock.XL, stock.XXL, stock.XXXL)
