...
func GetOrders() ([]models.Order, error) {
	var query = fmt.Sprintf("SELECT orders.id,product_id,orders.name,address,house,pin,city,phone,payment,total,status,product.name,product.price FROM orders LEFT JOIN product ON orders.product_id = product.id;")
	...
	for result.Next() {
		...
		result.Scan(&order.Id, &order.ProductId, &order.Name, &order.Address, &order.House, &order.Pin, &order.City, &order.Phone, &order.Payment, &order.Total, &order.Status, &order.ProductName, &order.ProductPrice)
